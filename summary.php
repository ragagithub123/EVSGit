...
if(!empty($_SESSION['agentid'])){
   ...
   $totaltimeExtras =array();
	$postinvoices =array();

	$postinvoicesExtra =array();

	$postinvoicesProfiles =array();

	$postinvoicesTime =array();

	$postinvoicestravel =array();

	$postinvoicespaint =array();

	$wsvalueTot =array();

	$wsvalueExtarstot =array();
	...
	if(isset($_POST['invicetype'])){

		if($_POST['invicetype'] == "extras")

		$db->joinquery("INSERT INTO `supplierInvoices`(`locationid`, `agentId`,`type`, `supplier`, `descrption`, `date`, `invoice`, `units`, `total`) VALUES (".$Locationid.",".$_SESSION['agentid'].",'".$_POST['invicetype']."','".$_POST['supplierextra']."','".$_POST['descriptionextra']."','".$_POST['dateextra']."','".$_POST['invoiceextra']."','".$_POST['unitsextra']."','".$_POST['totalextra']."')");

		
		else
       
		$db->joinquery("INSERT INTO `supplierInvoices`(`locationid`, `agentId`,`type`, `supplier`, `descrption`, `date`, `invoice`, `units`, `total`) VALUES (".$Locationid.",".$_SESSION['agentid'].",'".$_POST['invicetype']."','".$_POST['supplierglass']."','".$_POST['descriptionglass']."','".$_POST['dateglass']."','".$_POST['invoiceglass']."','".$_POST['unitsglass']."','".$_POST['totalglass']."')");
	...
	if(isset($_REQUEST['invoiceid'])){
		...
		if(!empty($_POST['supplier'])&&!empty($_POST['description'])&&!empty($_POST['dateglass'])&&!empty($_POST['invoiceglass'])&&!empty($_POST['units'])&&!empty($_POST['total']))

               
		$db->joinquery("UPDATE supplierInvoices SET supplier='".$_POST['supplier']."',descrption='".$_POST['description']."',date='".$_POST['dateglass']."',invoice='".$_POST['invoiceglass']."',units='".$_POST['units']."',total='".$_POST['total']."' WHERE supplinvoiceid='".$_REQUEST['invoiceid']."'");
	...
	$getinvoiceglass = $db->joinquery("SELECT * FROM supplierInvoices WHERE locationid='".$Locationid."' AND agentId='".$_SESSION['agentid']."' AND type='glass'");

	 if(mysqli_num_rows($getinvoiceglass)>0){

		 while($rowinvoices = mysqli_fetch_assoc($getinvoiceglass)){

			 $totalamout[] = $rowinvoices['total'];

			 $postinvoices[] = $rowinvoices;
		 }
	}


	$getinvoiceextras = $db->joinquery("SELECT * FROM supplierInvoices WHERE locationid='".$Locationid."' AND agentId='".$_SESSION['agentid']."' AND type='extras'");

	 if(mysqli_num_rows($getinvoiceextras)>0){

		 while($row_invoices = mysqli_fetch_assoc($getinvoiceextras)){

			 $totalamoutExtra[] = $row_invoices['total'];

			 $postinvoicesExtra[] = $row_invoices;
		 }
	}

	$getinvoicprofile = $db->joinquery("SELECT * FROM supplierInvoices WHERE locationid='".$Locationid."' AND agentId='".$_SESSION['agentid']."' AND type='Profiles'");

	 if(mysqli_num_rows($getinvoicprofile)>0){

		 while($profile_invoices = mysqli_fetch_assoc($getinvoicprofile)){

			 $totalamoutProfiles[] = $profile_invoices['total'];

			 $postinvoicesProfiles[] = $profile_invoices;
		 }
	}

	
	$getinvoictime = $db->joinquery("SELECT * FROM supplierInvoices WHERE locationid='".$Locationid."' AND agentId='".$_SESSION['agentid']."' AND type='Time'");

	 if(mysqli_num_rows($getinvoictime)>0){

		 while($time_invoices = mysqli_fetch_assoc($getinvoictime)){

			 $totalamoutTime[] = $time_invoices['total'];

			 $postinvoicesTime[] = $time_invoices;
		 }
	}

	$getinvoicTravel = $db->joinquery("SELECT * FROM supplierInvoices WHERE locationid='".$Locationid."' AND agentId='".$_SESSION['agentid']."' AND type='Travel'");

	 if(mysqli_num_rows($getinvoicTravel)>0){

		 while($travel_invoices = mysqli_fetch_assoc($getinvoicTravel)){

			 $totalamouttravel[] = $travel_invoices['total'];

			 $postinvoicestravel[] = $travel_invoices;
		 }
	}

	$getinvoicpaint = $db->joinquery("SELECT * FROM supplierInvoices WHERE locationid='".$Locationid."' AND agentId='".$_SESSION['agentid']."' AND type='Paint'");

	 if(mysqli_num_rows($getinvoicpaint)>0){

		 while($paint_invoices = mysqli_fetch_assoc($getinvoicpaint)){

			 $totalamoutpaint[] = $paint_invoices['total'];

			 $postinvoicespaint[] = $paint_invoices;
		 }
	}
	
	
	
	$get_details=$db->joinquery("SELECT location.locationid,location.`unitnum`,location.`street`,location.`suburb`,location.`city`,`location`.locationstatusid,location.notes,location.photoid,customer.customerid,customer.firstname,customer.lastname,customer.email,customer.phone,agent.labourrate,photo.width,photo.height FROM location LEFT JOIN photo ON location.photoid=photo.photoid ,agent,customer WHERE location.agentid=agent.agentid AND location.customerid=customer.customerid AND location.`locationid`=".$Locationid."");
	...
	$getroomcnt = $db->joinquery("SELECT COUNT(roomid) AS roomcnt FROM `room` WHERE `locationid`=".$Locationid."");
	
	
	$RoomCount = mysqli_fetch_array($getroomcnt);
	
	
	$getwindowcnt= $db->joinquery("SELECT COUNT(window.`windowid`) AS windowcnt FROM `window`,`room` WHERE window.roomid=room.roomid AND window.selected_product!='HOLD' AND room.locationid=".$Locationid."");
	
	
	$WindowCount = mysqli_fetch_array($getwindowcnt);
	
	
	$getpanelcnt = $db->joinquery("SELECT COUNT(panel.panelid) AS panelcount FROM `panel`,window,room WHERE panel.windowid=window.windowid AND window.roomid=room.roomid AND window.selected_product!='HOLD' AND room.locationid=".$Locationid."");
	
	
	$PanelCount = mysqli_fetch_array($getpanelcnt);
	...
	$productmargin = $db->joinquery("SELECT * FROM location_margins WHERE locationid=".$Locationid."");
	...
	if(mysqli_num_rows($getpanelglass)>0){
		...
		$totalRpriceGlass = array_sum($rrpvalue_glass);

		$totalWSValueGlass = array_sum($wsvalueTot);
	...
	$getSelected = $db->joinquery("SELECT DISTINCT(window.selected_product) FROM window,room WHERE window.roomid=room.roomid AND window.selected_product!='HOLD' AND room.locationid=".$Locationid."");
	...
	$gettotalPreploc = $db->joinquery("SELECT sum(`prep`) AS preptotal FROM `staffTrack` WHERE `agentid`='".$_SESSION['agentid']."' AND locationid='".$Locationid."'");
	...
	$gettotalInstallloc = $db->joinquery("SELECT sum(`install`) AS installtotal FROM `staffTrack` WHERE `agentid`='".$_SESSION['agentid']."' AND locationid='".$Locationid."'");
	...
	$gettotalMakeloc = $db->joinquery("SELECT sum(`make`) AS maketotal FROM `staffTrack` WHERE `agentid`='".$_SESSION['agentid']."' AND locationid='".$Locationid."'");
	...
	$gettotalExtraloc = $db->joinquery("SELECT sum(`extra`) AS extratotal FROM `staffTrack` WHERE `agentid`='".$_SESSION['agentid']."' AND locationid='".$Locationid."'");
	...
	$getTimeproduct  = $db->joinquery("SELECT DISTINCT(window.selected_product)FROM panel,room,window WHERE panel.windowid=window.windowid AND window.roomid=room.roomid AND window.selected_product!='HOLD' AND room.locationid=".$Locationid."");
	...
	if(mysqli_num_rows($getTimeproduct)>0){
		...
		while($row_time = mysqli_fetch_array($getTimeproduct)){
			...
			$timemargin = round($labourrate/$rowagnt['wagerate'],2);
	...
	$getExtras = $db->joinquery("SELECT window.selected_product,window.windowid,products.name,products.description,products.wsvalue,products.hours,products.costvalue,products.unittag,window_extras.quantity,window_extras.cost FROM window_extras,products,room,window WHERE window.roomid=room.roomid AND window.windowid=window_extras.windowid AND window_extras.productid=products.productid AND window.selected_product!='HOLD' AND room.locationid=".$Locationid."");
	...
	if(mysqli_num_rows($getExtras)>0){
		...
		while($row_extras = mysqli_fetch_array($getExtras)){
			...
			$extramargin = round(($labourrate/$rowagnt['wagerate']),2);
			
		
			$extrahours[] = $row_extras['hours'] *($row_extras['quantity']);
			
			// newchange 10-18-21
			
			$wsvalue_extras = round(($rowagnt['wagerate'] * $row_extras['hours'] * $row_extras['quantity']),2);



			// newchange 10-18-21
			
			$totaltimeExtras[] = round(($row_extras['hours'] * $row_extras['quantity']),2);
			
			
			$rprice_extras =round(($wsvalue_extras*($extramargin/100)*100),2);
			
			// newchange 10-18-21

			$totalRpriceExtras[] = $rprice_extras;
			
			$array_extras_time_wsvalue[] = round($wsvalue_extras,2);
			
			
			
			$array_rprice_extras[] = round($rprice_extras,2);
	...
	if($extraTotal!=0 || !empty($extraTotal)){
		...
		$extra_margin = round(($labourrate/$rowagnt['wagerate']),2);
		...
		if(empty($totaltimeExtras)) $extra_hour_total = 0;
		
		
		else $extra_hour_total = array_sum($totaltimeExtras);
		
		
		if($extraTotal > $extra_hour_total)
		
		$post_extras['colourbk_extra'] = '#FF0000';
		...
		$wsvalueExtarstot[] = round(($rowagnt['wagerate'] * $extraTotal),2) ;
		
		
		$post_extras['pdt_rprice_extras'] = round((( $row_time['pdt_wsvalue_extras'] * ($extra_margin/100)) * (100 -0)),2);

		$RPriceTotalExtra[] = round((( $row_time['pdt_wsvalue_extras'] * ($extra_margin/100)) * (100 -0)),2);

		$ttoalRpriceExtrs = array_sum($RPriceTotalExtra);

		$wsvalueExtrasSum  = array_sum($wsvalueExtarstot);
	...
	$getTravel = $db->joinquery("SELECT `number_staff`,`distance`,`return_trip`,`travel_status` FROM `location` WHERE locationid=".$Locationid."");
	...
	if(mysqli_num_rows($getTravel)>0){
		...
		if($row_travel['travel_status'] == 1){
			...
			$getPanels = $db->joinquery("SELECT window.selected_product,window.windowid FROM window,room WHERE window.selected_product!='HOLD' AND window.roomid=room.roomid AND room.locationid='".$Locationid."'");
	...
	$getColor = $db->joinquery("SELECT panel.colourid FROM panel,colours,room,window WHERE panel.colourid = colours.colourid AND panel.windowid=window.windowid AND window.roomid=room.roomid AND window.selected_product!='HOLD' AND room.locationid=".$Locationid."");
	...
	if(count($colourid)>0){
		...
		$paint_timemargin = round(($labourrate/$rowagnt['wagerate']),2);
		...
		$material_wsCost = round(($total_costcount/$costmargin),2);
		
		
		
		$rateWsval = round(($material_wsCost/$total_panelcount),2);
	...
	$hourlyreturn = round((($totalmargin/$ttoal_man_insatll_hrs)/1.15),2);
