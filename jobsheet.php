...
if(!empty($_SESSION['agentid'])){
	...
	$getworksheet = $db->joinquery("SELECT * FROM worksheet_settings WHERE locationid=".$Locationid." AND type='jobsheet'");
	...
	$getweektimesheet = $db->joinquery("SELECT * FROM  job_measure_worksheet WHERE locationid=".$Locationid." AND dates!='0000-00-00' AND type='jobsheet' ORDER BY week ASC");
...
panel.styleid=paneloption_style.styleid AND panel.safetyid=paneloption_safety.safetyid AND panel.astragalsid=paneloption_astragal.astragalsid AND panel.glasstypeid=paneloption_glasstype.glasstypeid AND panel.conditionid=paneloption_condition.conditionid AND panel.windowid=".$row_window['windowid']."");
	...
	$row_panel['ProfileX'] = 	$row_panel['width'] + $profileX;
		...
		if(($row_panel['center']) > ($row_panel['height'])){
				...
				$row_panel['ProfileY'] = ($row_panel['center'])+$profileY;
			...
			$row_panel['ProfileY']=($row_panel['height']) + ($profileY);
	...
	$getworkflow = $db->joinquery("SELECT * FROM worksheet_workflow WHERE locationid=".$Locationid." AND roomid =".$row_window['roomid']." AND panelid=".$row_panel['panelid']." AND type='jobsheet'");
