...
function set($queryString, $params)
{
	...
	try {
		...
		$distance_api=file_get_contents('https://maps.googleapis.com/maps/api/distancematrix/json?origins='.$source_address.',&destinations='.$destination_location.'&departure_time=now&key=xxxxxxxxxx');
...
function CalcPanel($mysqli, $panelId, $calcParams, $locationId)
{
 ...
 $panEVSx3 = round(((($calcParams['evsx3glassrate'] + ($panel['safetyvalue'] / 2)) * $m2) + ($calcParams['evsmaterials'] * $lm) + ($panEVSLabour * $calcParams['labourrate'])) * $calcParams['agentmargin'], 2);*/
	...
	$getagent = "SELECT SGUrate,IGUx2rate,IGUx3rate,EVSx2rate,EVSx3rate FROM agent WHERE agentid='" . $location_agent['agentid'] . "'";
	if (!($query = $mysqli->query($getagent)))
		ThrowDBException($getagent, $mysqli->error);
...
function Updatelatlong($locationsearch, $locationId, $mysqli)
{
	...
	$res = file_get_contents("https://maps.googleapis.com/maps/api/geocode/json?address='" . $location . "'&key=xxxxxxxxxxxxxxxxxxx");
