...
function create($queryString, $params) {
	...
	try {
		...
		foreach($params['data']['photos'] as $photo) {
			...
			fclose($fh);
			# window photo
			if($windowId > 0 && $photo['type']=='before'){ 
				$querySQL = "INSERT INTO window_photo (photoid, windowid) VALUES ($photoId, $windowId)";
			}
				
				else if($windowId > 0 && $photo['type']=='after'){ 
				$querySQL = "INSERT INTO window_after_photo (photoid, windowid) VALUES ($photoId, $windowId)";
				}
				
			# hazard photo
			elseif($hazardField != null) {
