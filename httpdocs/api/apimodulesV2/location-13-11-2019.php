...
function set($queryString, $params) {
	...
	try {
		...
		$distance_api=file_get_contents('https://maps.googleapis.com/maps/api/distancematrix/json?origins='.$source_address.',&destinations='.$destination_location.'&departure_time=now&key=xxxxx');
...
$panDGLabour = (($panel['styledgvalue'] + $m2 + ($panel['conditionvalue'] * $lm) + ($panel['astragalvalue'] * 2)));
$panEVSLabour = (($panel['styleevsvalue'] + $m2) + ($panel['conditionvalue'] * $lm * 0.3) + ($panel['astragalvalue'] * $panel['conditionvalue']));
$panSDG = round((((($agent['SGUrate'] + $panel['safetyvalue'] + $panel['typevalue']) * $m2) * $location_margins['igumargin']) + ((($calcParams['dgmaterials'] * $lm) + ($panel['IGUassemble']* $lm)) *$location_margins['productmargin']) + ($panDGLabour*$location_margins['labourrate'])) , 2);
$panMAXe = round((((($agent['IGUx2rate'] + $panel['safetyvalue'] + $panel['typevalue']) * $m2) *$location_margins['igumargin']) + ((($calcParams['dgmaterials'] * $lm)+ ($panel['IGUassemble']* $lm)) *$location_margins['productmargin']) + ($panDGLabour*$location_margins['labourrate'])) , 2);
$panXCLe = round((((($agent['IGUx3rate'] + $panel['safetyvalue'] + $panel['typevalue']) * $m2) *$location_margins['igumargin']) + ((($calcParams['dgmaterials'] * $lm) + ($panel['IGUassemble']* $lm)) *$location_margins['productmargin']) + ($panDGLabour*$location_margins['labourrate'])) , 2);
//$panEVSx2 = round(((((($agent['SGUrate']+($panel['safetyvalue'] / 2))* $m2)+($calcParams['evsmaterials'] * $lm)+($panel['EVSassemble']))* $location_margins['evsmargin'])+($panEVSLabour * $location_margins['labourrate'])),2);
//$panEVSx3=round(((((($agent['IGUx2rate']+($panel['safetyvalue'] / 2))* $m2+($calcParams['evsmaterials'] * $lm)+($panel['EVSassemble']))* $lm)*$location_margins['evsmargin'])+($panEVSLabour * $location_margins['labourrate'])),2);
//$panEVSx2 = round((((((($agent['SGUrate']+($panel['safetyvalue'] / 2))* $m2)+($calcParams['evsmaterials'] * $lm)+($panel['EVSassemble']))* $lm)* $location_margins['evsmargin'])+($panEVSLabour * $location_margins['labourrate'])),2);
//$panEVSx3=round((((((($agent['IGUx2rate']+($panel['safetyvalue'] / 2))* $m2)+($calcParams['evsmaterials'] * $lm)+($panel['EVSassemble']))* $lm)*$location_margins['evsmargin'])+($panEVSLabour * $location_margins['labourrate'])),2);
$panEVSx2 = round (((($agent['SGUrate']  +($panel['safetyvalue'] / 2))* $m2) + ((($calcParams['evsmaterials'] + $panel['EVSassemble'])* $lm)* $location_margins['evsmargin'])+ ($panEVSLabour * $location_margins['labourrate'])),2);
$panEVSx3 =round  (((($agent['IGUx2rate']+($panel['safetyvalue'] / 2))* $m2) +((($calcParams['evsmaterials'] + $panel['EVSassemble'])* $lm)* $location_margins['evsmargin'])+ ($panEVSLabour * $location_margins['labourrate'])),2);
