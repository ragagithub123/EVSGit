...
# globals
global $modules;
$modules = array("session", "locations", "location", "room", "window", "panel", "prodlocation", "photo", "customer", "windowtypes", "windowoptions", "quote","settings","products","frametype","tracking");
...
try {
	$urlFields = explode("/", $_SERVER['REQUEST_URI']);
	...
	if(!in_array($module, $modules) || !file_exists("apimodules/$module.php"))
		...
		if(!in_array($module, $modules) || !file_exists("apimodulesV2/$module.php"))
		throw new Exception('Invalid or missing module');
	else
		require_once("apimodulesV2/$module.php");
	}
	...
	$postData['gProfilePhotoURL']=$gProfilePhotoURL;
	$postData['gProdcutURL']=$gWindowOptionProductsURL;
	$postData['gProductDir']=$gWindowOptionProductsDir;
	$postData['gFrameURL']=$gPanelOptionsPhotoURL;
	$postData['gFrameDir']=$gPanelOptionsPhotoDir;
	$postData['AgentDir']=$gLogoPhotoDir;
	$postData['Agenturl']=$gLogoPhotoURL;
...
function AgentHasPermission($mysqli, $agentId, $table, $tableKeyId) {
	...
	if(!($query = $mysqli->query($querySQL))) {
		...
		// throw new Exception('DatabaseError-1'. ' Query: ' .$querySQL. ' Error: '. $mysqli->error);
...
function ThrowDBException($querySQL, $mysqlError) {
	...
	// throw new Exception('DatabaseError-2' . ' Query: ' . $querySQL . ' Error: ' . $mysqlError);
	throw new Exception('DatabaseError');
}
...
function DeletePhotos($mysqli, $photoDir, $photoIds, $tables=array('photo', 'window_photo','window_after_photo')) {
	...
	foreach($tables as $table) {
		if($table == 'window_photo' || $table == 'window_after_photo'){
			$chk_photo="SELECT photoid FROM $table WHERE photoid IN (". implode(',', $photoIds). ")";
	if(!($query_pht = $mysqli->query($chk_photo)))
		ThrowDBException($chk_photo, $mysqli->error);	
			if($query_pht->num_rows == 0){
				 if($table == 'window_photo')
					$querySQL = "DELETE FROM window_after_photo WHERE photoid IN (". implode(',', $photoIds). ")";
    else
				$querySQL = "DELETE FROM window_photo WHERE photoid IN (". implode(',', $photoIds). ")";
			}
		}
		else{
