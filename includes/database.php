...
class Database
{
	...
	{
		session_start();
		// $connection = mysql_connect(DB_SERVER, DB_USERNAME, DB_PASSWORD) or die('Oops connection error -> ' . $this->connection->error);
		// mysql_select_db(DB_DATABASE, $connection) or die('Database error -> ' . $this->connection->error);
	...
	public function ins_rec($tab, $array, $disp = false)
	{
		...
		if ($disp)
			echo $qry;
		$err = mysqli_query($this->connection, $qry);
		if (!$err) {
			echo $this->connection->error . " - <b>" . $qry . "</b>";
			return false;
		} else {
	...
	public function joinquery($query)
	{
		...
		if (!$res)
			echo $this->connection->error . "-<b>" . $query . "</b>";
		if (mysqli_num_rows($res) > 0)
			return $res;
		else
			return false;
	...
	public function sel_img_hea($tab, $field, $temp, $disp = false)
	{
	...
	public function sel_img_hea_new($tab, $field, $temp, $exist_img, $disp = false)
	{
	...
	public function sel_rec_group($tab, $id, $uid, $limit = "", $disp = false)
	{
	...
	{
		$res = $this->sel_rec($tab, $fields, $where, $orderby, $order, $limit, $disp);
		if ($res) {
			$val = mysqli_fetch_assoc($res);
			return $this->strip_slashes($val[$fields]);
		} else
			return false;
	}
	...
	{
		$res_single = $this->sel_rec($tab, $fields, $where, $orderby, $order, $limit, $disp);
		//echo mysqli_num_rows($res_single); exit;
		if ($res_single != false && mysqli_num_rows($res_single) > 0) {
			//echo "test"; exit;
			return $this->strip_slashes(mysqli_fetch_assoc($res_single));
		} else
			return false;
	}
	...
	public function del_rec_slide($tab, $where, $disp=false)
	{
		$qry = "delete from $tab where $where";
		if ($disp)
			echo $qry;
...
class Encryption
{
	...
	public  function encode($string, $key = 'PrivateKey', $secret = 'SecretKey', $method = 'AES-256-CBC')
	{
		// hash
		$key = hash('sha256', $key);
	...
	public function decode($string, $key = 'PrivateKey', $secret = 'SecretKey', $method = 'AES-256-CBC')
	{
		// hash
		$key = hash('sha256', $key);
		// create iv - encrypt method AES-256-CBC expects 16 bytes
